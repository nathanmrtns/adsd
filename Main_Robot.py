...
class TesterADSD(object):
    ...
    def start(self):
        ...
        if(self.repetitions == 10):
            self.paced_shooter(self.url, self.repetitions, self.mode, 6) #10 requests per minute
        else:
            self.paced_shooter(self.url, self.repetitions, self.mode, 0.24) #250 requests per minute
        print "done"

    def paced_shooter(self, url, repetitions, command, wait_time):
        counter = 0
...
if __name__ == '__main__':
    tester = TesterADSD(1, 5, output='output.csv')
